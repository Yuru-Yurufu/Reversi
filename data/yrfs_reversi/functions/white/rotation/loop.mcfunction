...
# 一つ先のマスが黒ならループ
  $execute positioned $(position) as @e[type=item_display,tag=reversi_disc,distance=..0.01,sort=nearest,limit=1] if data entity @s {item:{tag:{CustomModelData:1}}} run function yrfs_reversi:white/rotation/loop {position:"$(position)"}
