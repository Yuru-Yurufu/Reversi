...
#
# @within function
#           yrfs_reversi:white/player/turn_start
...
# ターン交代
  execute unless data storage yrfs_reversi: {root:{game_status:{turn:"kill_process"}}} run data modify storage yrfs_reversi: root.game_status.turn set value "waiting_white"
  execute unless data storage yrfs_reversi: {root:{game_status:{turn:"kill_process"}}} if data storage yrfs_reversi: {root:{game_status:{is_cpu:{black:false}}}} run schedule function yrfs_reversi:black/player/turn_start 20
  execute unless data storage yrfs_reversi: {root:{game_status:{turn:"kill_process"}}} if data storage yrfs_reversi: {root:{game_status:{is_cpu:{black:true}}}} run schedule function yrfs_reversi:black/cpu/virtual_turn/init 20
