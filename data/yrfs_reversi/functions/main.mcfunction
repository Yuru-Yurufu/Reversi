...
# 操作パネルのボタンに視点を合わせた時用の処理
  execute at @e[type=marker,tag=reversi_gui_center,limit=1] unless entity @a[distance=..4] as @e[type=text_display,tag=reversi_gui_show_underline_] run function yrfs_reversi:gui/remove_underline/init
  execute at @e[type=marker,tag=reversi_gui_center,limit=1] if entity @a[distance=..4] run function yrfs_reversi:gui/find_gaze_point/loop
