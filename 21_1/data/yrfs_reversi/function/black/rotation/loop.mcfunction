...
# 一つ先のマスが白ならループ
  $execute positioned $(position) as @e[type=item_display,tag=reversi_disc,distance=..0.01,sort=nearest,limit=1] if data entity @s {item:{components:{"minecraft:custom_model_data":2}}} run function yrfs_reversi:black/rotation/loop {position:"$(position)"}
