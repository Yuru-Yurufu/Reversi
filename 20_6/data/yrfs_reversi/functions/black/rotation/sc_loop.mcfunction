...
# ひっくり返すアニメーション
  execute as @e[type=item_display,tag=reversi_disc,tag=reversi_rotation,scores={reversi_rotation_delay=0}] at @s run function yrfs_reversi:black/rotation/tick_0
  ...
  execute as @e[type=item_display,tag=reversi_disc,tag=reversi_rotation,scores={reversi_rotation_delay=-5}] at @s run function yrfs_reversi:black/rotation/tick_5

# 全部ひっくり返したらターン終了処理をする
  execute unless entity @e[type=item_display,tag=reversi_disc,tag=reversi_rotation] run function yrfs_reversi:black/turn_end

# 次tickのアニメーション
  execute if entity @e[type=item_display,tag=reversi_disc,tag=reversi_rotation] run schedule function yrfs_reversi:black/rotation/sc_loop 1
