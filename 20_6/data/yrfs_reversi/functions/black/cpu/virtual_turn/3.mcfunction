...
# 白のターン
#
# @within function yrfs_reversi:black/cpu/virtual_turn/2
...
# 55ターン目以降は石の数も考慮に入れる
  execute if score turn reversi matches 55.. run function yrfs_reversi:black/cpu/count_disc_3
...
# 1ターン目に戻る
  execute unless entity @e[type=marker,tag=reversi_marker,tag=reversi_cpu_can_place_here_,tag=!reversi_cpu_now_placed_here_] run schedule function yrfs_reversi:black/cpu/virtual_turn/1_end 1
  execute if entity @e[type=marker,tag=reversi_marker,tag=reversi_cpu_can_place_here_,tag=!reversi_cpu_now_placed_here_] run schedule function yrfs_reversi:black/cpu/virtual_turn/1_loop 2
