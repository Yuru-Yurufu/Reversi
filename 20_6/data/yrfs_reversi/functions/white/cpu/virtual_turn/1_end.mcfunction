...
# (開放度+α)が一番低くなる場所に置く
  ...
  execute at @e[type=marker,tag=reversi_marker,tag=reversi_cpu_now_placed_here,limit=1] run data modify entity @e[type=item_display,tag=reversi_disc,distance=..1.1,limit=1] item.components."minecraft:custom_model_data" set value 2
